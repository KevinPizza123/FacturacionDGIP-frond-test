...
export class CentroCostoService {
  private apiUrl = 'http://localhost:8080/centrocosto/listarCentroCosto';
  private api= 'http://localhost:8080/centrocosto'

  constructor(private http: HttpClient) { }

  
   getCentroCosto(): Observable<CentroCostoDto[]> {
     return this.http.get<CentroCostoModel>('http://localhost:8080/centrocosto/listarCentroCosto').pipe(
  ...
  eliminarCentroCosto(id: number): Observable<CentroCostoDto[]> {
    return this.http.delete<CentroCostoModel>('http://localhost:8080/centrocosto/eliminarCentroCosto/' + id ).pipe(
