...
export class TarjetaHttpServiceService {
  ...
  private apiUrl = 'http://localhost:8080/Tarjeta/listTarjeta';

  private url = 'http://localhost:8080/Tarjeta';

  constructor(private http: HttpClient) { }

  
  getTarjeta(): Observable<TarjetaDTO[]> {
    return this.http.get<TarjetaModel>('http://localhost:8080/Tarjeta/listTarjeta').pipe(
  ...
  eliminarTarjeta(id: number): Observable<TarjetaDTO[]> {
    return this.http.delete<TarjetaModel>('http://localhost:8080/Tarjeta/deleteTarjeta/' + id ).pipe(
