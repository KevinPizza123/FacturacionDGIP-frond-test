...
export class TipoconceptoService {
  ...
  private apiUrl = 'http://localhost:8080/Concepto/listarConcepto';

  private url = 'http://localhost:8080/Concepto';

  constructor(private http: HttpClient) { }

  
  getListado(): Observable<TipoConceptoDTO[]> {
    return this.http.get<TipoConceptoModel>('http://localhost:8080/Concepto/listarConcepto').pipe(
  ...
  eliminarTipoConcepto(id: number): Observable<TipoConceptoDTO[]> {
    return this.http.delete<TipoConceptoModel>('http://localhost:8080/Concepto/eliminaConcepto/' + id ).pipe(
